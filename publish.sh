...
##
BUCKET='s3://nicksloan.com'
SITE_DIR='public/'
export AWS_DEFAULT_PROFILE='nicksloan'
...
aws s3 sync --size-only --delete --exclude '*.*' --include '*.png' --include '*.jpg' --include '*.ico' --cache-control='max-age=300' $SITE_DIR $BUCKET

yellow '--> Uploading fonts (woff, eot, svg, otf, ttf)'
aws s3 sync --size-only --delete --exclude '*.*' --include '*.woff' --include '*.eot' --include '*.svg' --include '*.ttf' --include '*.otf' --cache-control='max-age=300' $SITE_DIR $BUCKET
